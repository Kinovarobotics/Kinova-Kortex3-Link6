...
# This example fetches the information from all devices and uses json_format to convert the protobuf messages to dictionaries
def example_routed_device_config(device_manager, device_config, base_client):
    ...
    for dev in all_devices_info.device_handle:
        ...
        device_info.update(
            json_format.MessageToDict(
                device_config.GetDeviceType(dev.device_identifier, options)
            )
        )
        device_info.update(
            json_format.MessageToDict(base_client.GetFirmwareBundleVersions())
        )
        device_info.update(
            json_format.MessageToDict(
                device_config.GetBootloaderVersion(dev.device_identifier, options)
            )
        )
        device_info.update(
            json_format.MessageToDict(
                device_config.GetModelNumber(dev.device_identifier, options)
            )
        )
        device_info.update(
            json_format.MessageToDict(
                device_config.GetPartNumber(dev.device_identifier, options)
            )
        )
        device_info.update(
            json_format.MessageToDict(
                device_config.GetPartNumberRevision(dev.device_identifier, options)
            )
        )
        device_info.update(
            json_format.MessageToDict(
                device_config.GetSerialNumber(dev.device_identifier, options)
            )
        )

        # Get hexadecimal representation of MAC address
        macAddress_hexstr = ""
        for b in device_config.GetMACAddress(
            dev.device_identifier, options
        ).mac_address:
        ...
        print(
            "-- {}: id = {} --".format(
                Common_pb2._DEVICETYPES.values_by_number[dev.device_type].name,
...
def main():
    ...
    with utilities.DeviceConnection.createMqttConnection(args) as router:
        ...
        # Example core
        example_routed_device_config(device_manager, device_config, base_client)
